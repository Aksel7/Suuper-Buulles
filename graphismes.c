 ...
 *  Fallback procédural pour bulles, fireballs, bonus, boss
 * ========================================================= */
...
#include <string.h>
#include <stdbool.h>
#include <direct.h>   /* _chdir Windows/MinGW */
...
{
    if (!img) { clear_to_color(dest, makecol(0, 0, 0)); return; }
                                ...
                                int couleur, bool centre)
{
    ...
    stretch_sprite(dest, tmp, px + taille, y + taille, lw*taille, lh*taille);
    clear_to_color(tmp, makecol(255, 0, 255));
...
void graphique_init_allegro(void)
{
    ...
    /* Force le répertoire courant vers cmake-build-debug
       pour que "assets/" soit trouvé correctement          */
    _chdir("C:/informatique ing 1v2/projet2/superpong/cmake-build-debug");
                                   ...
                                   const char *dossier)
{
    ...
    strncpy(s_asset_dir, "C:/informatique ing 1v2/projet2/superpong/cmake-build-debug/assets", 255);
    (void)dossier;
    s_asset_dir[255] = '\0';
                                   ...
                                   int hover_retour)
{
    ...
    dessiner_bouton(buf,res->btn_nouvelle_idle,res->btn_nouvelle_hover,270,270,hover[0]);
    dessiner_bouton(buf,res->btn_charger_idle, res->btn_charger_hover, 270,370,hover[1]);
...
{
    int cx=WINDOW_W/2;
    int blanc=makecol(255,255,255), or_=makecol(255,200,0), rouge=makecol(220,60,60);
    (void)hover_retour;
    dessiner_fond(buf, res->fond_menu, WINDOW_W, WINDOW_H);
    ...
    switch (etat) {
        case ETAT_MENU_REGLES:
            ...
            graphique_textout_stylise(buf,"- Q/D pour se deplacer",cx,265,2,blanc,true);
                                      ...
                                      const char *pseudo)
{
    char aff[64];
    int cx=WINDOW_W/2;
    BITMAP *fond = res->saisie_pseudo_fond ? res->saisie_pseudo_fond : res->fond_menu;
    dessiner_fond(buf, fond, WINDOW_W, WINDOW_H);
    graphique_textout_stylise(buf,"ENTREZ VOTRE PSEUDO",cx,240,4,makecol(255,200,0),true);
    snprintf(aff,sizeof(aff),"%s_",pseudo);
    graphique_textout_stylise(buf,aff,cx,350,4,makecol(255,255,255),true);
    graphique_textout_stylise(buf,"Appuyez sur ENTREE pour valider",cx,460,2,makecol(255,255,255),true);
}
...
void graphique_dessiner_choix_joueurs(BITMAP *buf,
                                      ...
                                      int mx, int my)
{
    int cx=WINDOW_W/2, bw=300, bh=70;
    ...
    BITMAP *fond = res->choix_joueurs_fond ? res->choix_joueurs_fond : res->fond_menu;
    dessiner_fond(buf, fond, WINDOW_W, WINDOW_H);
    graphique_textout_stylise(buf,"NOMBRE DE JOUEURS",cx,150,5,makecol(255,200,0),true);
    graphique_textout_stylise(buf,hov1?"> 1 JOUEUR <":"1 JOUEUR",cx,y1+20,4,makecol(255,255,255),true);
    graphique_textout_stylise(buf,"Q/D + ESPACE",cx,y1+50,2,makecol(180,180,180),true);
    graphique_textout_stylise(buf,hov2?"> 2 JOUEURS <":"2 JOUEURS",cx,y2+20,4,makecol(255,255,255),true);
    graphique_textout_stylise(buf,"J2 : Fleches + Clic",cx,y2+50,2,makecol(180,180,180),true);
    graphique_textout_stylise(buf,hovr?"> RETOUR <":"RETOUR",cx,yr+15,3,makecol(255,80,80),true);
}
                                    ...
                                    const Joueur *j, int frame, int j2)
{
    ...
    flip=(j->vx<-0.1f);
    if (j2) {
        img=(j->vx>0.1f||j->vx<-0.1f)?res->joueur2_run[anim]:res->joueur2_idle;
        if (!img) img=res->joueur2_idle;
        if (!img) img=res->joueur_idle;
    } else {
        img=(j->vx>0.1f||j->vx<-0.1f)?res->joueur_run[anim]:res->joueur_idle;
        if (!img) img=res->joueur_idle;
    }
    if (!img) return;
    if (flip) {
        ...
        if (!tmp) return;
        clear_to_color(tmp,makecol(255,0,255));
        masked_stretch_blit(img,tmp,0,0,img->w,img->h,0,0,PLAYER_W,PLAYER_H);
                             ...
                             const Bulle *bulles)
{
    ...
    for (b=bulles;b;b=b->suivante) {
        ...
        t=b->taille; if(t<1)t=1; if(t>3)t=3;
        r=BULLE_RAYON_BASE*t;
        if (b->choc_timer>0&&res->bulle_choc[t]) img=res->bulle_choc[t];
        else if (b->lance_eclairs&&res->bulle_eclair[t]) img=res->bulle_eclair[t];
        else img=res->bulle[t];
        if (img) {
            dessiner_image(buf,img,(int)b->x-r,(int)b->y-r,2*r,2*r);
        } else {
            ...
            if(b->choc_timer>0){int rc=r+(8-b->choc_timer);circle(buf,cx,cy,rc,makecol(255,255,255));}
        }
    }
}

static void dessiner_eclairs_jeu(BITMAP *buf, const Ressources *res,
                                  const Eclair *e, int frame)
{
    int anim=(frame/4)%4;
    for (;e;e=e->suivant) {
        if (!e->actif) continue;
        BITMAP *img=res->eclair_anim[anim]?res->eclair_anim[anim]:res->eclair_spr;
        if (img) dessiner_image(buf,img,(int)e->x-16,(int)e->y,32,80);
        else {
...
static void dessiner_bonus_jeu(BITMAP *buf, const Ressources *res,
                                const BonusItem *bon)
{
    int bcols[NB_TYPES_BONUS]={makecol(80,180,255),makecol(80,255,120),
        makecol(255,200,30),makecol(180,255,180),makecol(255,80,200)};
    for (;bon;bon=bon->suivant) {
        if (!bon->actif||bon->type<0||bon->type>=NB_TYPES_BONUS) continue;
        if (res->bonus[bon->type])
            dessiner_image(buf,res->bonus[bon->type],(int)bon->x-18,(int)bon->y-18,36,36);
        else {
...
static void dessiner_fireballs_jeu(BITMAP *buf, const Ressources *res,
                                    const Fireball *pool, int nb)
{
    ...
    for (i=0;i<nb;i++) {
        ...
        if (res->fireball[anim])
            dessiner_image(buf,res->fireball[anim],(int)pool[i].x-16,(int)pool[i].y-16,32,32);
        else {
...
static void dessiner_boss_jeu(BITMAP *buf, const Ressources *res,
                               const Boss *boss, int frame)
{
    BITMAP *img; int bx,by,anim;
    if (!boss||!boss->actif) return;
    bx=(int)boss->x-50; by=(int)boss->y-50;
    anim=(frame/8)%4;
    if (boss->hit_flash>0&&res->boss_hit) img=res->boss_hit;
    else if (res->boss_run[anim]) img=res->boss_run[anim];
    else img=res->boss_idle;
    if (img) dessiner_image(buf,img,bx,by,100,100);
    else { circlefill(buf,(int)boss->x,(int)boss->y,50,makecol(180,30,30)); }
    {
        int bw2=200,bx2=WINDOW_W/2-100,by2=GAME_H-HUD_H-20;
        ...
        graphique_textout_stylise(buf,"BOSS",WINDOW_W/2,by2-18,2,makecol(255,80,80),true);
    }
}
...
{
    int hy=WINDOW_H-HUD_H; char tmp[64]; int col_temps; int i;
    if (res->hud_fond) dessiner_image(buf,res->hud_fond,0,hy,WINDOW_W,HUD_H);
    else { rectfill(buf,0,hy,WINDOW_W,WINDOW_H,makecol(10,10,20)); hline(buf,0,hy,WINDOW_W-1,makecol(255,200,0)); }
    ...
    for (i = 0; i < 3; i++) {
        int hx = 20 + i * 22;
        int hy2 = hy + 50;
        int col = (i < j->vies) ? makecol(255,50,50) : makecol(80,20,20);
        rectfill(buf, hx+2, hy2,    hx+6,  hy2+4,  col);
        rectfill(buf, hx+10,hy2,    hx+14, hy2+4,  col);
        rectfill(buf, hx,   hy2+2,  hx+16, hy2+10, col);
    ...
    if (j2) {
        ...
        /* Coeurs vies J2 */
        for (i = 0; i < 3; i++) {
            int hx = WINDOW_W - 220 + i * 22;
            int hy2 = hy + 50;
            int col = (i < j2->vies) ? makecol(80,200,255) : makecol(20,60,80);
            rectfill(buf, hx+2, hy2,    hx+6,  hy2+4,  col);
            rectfill(buf, hx+10,hy2,    hx+14, hy2+4,  col);
            rectfill(buf, hx,   hy2+2,  hx+16, hy2+10, col);
            rectfill(buf, hx+2, hy2+10, hx+14, hy2+14, col);
                             ...
                             const EtatNiveau *niveau)
{
    ...
    if(idx<0)idx=0; if(idx>=NB_NIVEAUX)idx=NB_NIVEAUX-1;
    dessiner_fond(buf,res->fond_niveaux[idx],WINDOW_W,GAME_H);
    dessiner_bonus_jeu(buf,res,bonus);
    dessiner_eclairs_jeu(buf,res,eclairs,frame);
    dessiner_fireballs_jeu(buf,res,fireballs,nb_fireballs);
    dessiner_bulles(buf,res,bulles);
    dessiner_projectiles(buf,res,projs);
    if (projs2) dessiner_projectiles(buf,res,projs2);
    dessiner_boss_jeu(buf,res,boss,frame);
...
void graphique_dessiner_pause(BITMAP *buf, const Ressources *res, int hover[2])
{
    int cx=WINDOW_W/2;
    ...
    graphique_textout_stylise(buf,hover[0]?"> Reprendre <":"Reprendre",cx,390,3,makecol(255,255,255),true);
    graphique_textout_stylise(buf,hover[1]?"> Menu Principal <":"Menu Principal",cx,480,3,makecol(255,255,255),true);
}
                                   ...
                                   const char *pseudo, int hover[4])
{
    ...
    for (i=0;i<4;i++) {
        ...
        graphique_textout_stylise(buf,label,cx,370+i*70,3,makecol(255,255,255),true);
    }
}
                                   ...
                                   int score, int hover[2])
{
    ...
    graphique_textout_stylise(buf,hover[0]?"> Rejouer <":"Rejouer",cx,460,3,makecol(255,255,255),true);
    graphique_textout_stylise(buf,hover[1]?"> Menu Principal <":"Menu Principal",cx,540,3,makecol(255,255,255),true);
}