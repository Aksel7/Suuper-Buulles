         ...
         const char *pseudo, int niveau_depart, int nb_joueurs)
{
    ...
    while (etat != J_MENU && etat != J_QUITTER) {
        ...
        if (joueur.tir_cooldown > 0)
            joueur.tir_cooldown--;

        if (joueur2.tir_cooldown > 0)
